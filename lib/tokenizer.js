...
import sanitizeHtml from './sanitize-html-browser';
...
export default function sentences(text, user_options) {
    ...
    } else if (options.extend_abbreviations) {
        Match.setAbbreviations(options.abbreviations);
    }
    ...
    if (options.html_boundaries) {
        let html_boundaries_regexp = "(<br\\s*\\/?>|<\\/(" + options.html_boundaries_tags.join("|") + ")>)";
        let re = new RegExp(html_boundaries_regexp, "g");
    ...
    let result = sentences.slice(1).reduce(function (out, sentence) {
        let lastSentence = out[out.length - 1];
